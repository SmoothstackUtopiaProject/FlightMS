	...
	public List<Flight> applySearch(List<Flight> flights, HashMap<String, String> filterMap) {
		...
		if(filterMap.keySet().contains(searchTerms)) {
			...
			for(Flight flight : flights) {
				...
				//try {
					//String flightAsString = mapper.writeValueAsString(flight).toLowerCase();
					String flightAsString =  ("flightId=\"" + flight.getFlightId() + "\"" + 
					"routeId=\"" + flight.getFlightRouteId() + "\"" + 
					"seatingId=\"" + flight.getFlightSeatingId() + "\"" + 
					"airplaneId=\"" + flight.getFlightAirplaneId() + "\"").toLowerCase() ;
					...
					/*
					 * } catch(JsonProcessingException err){ containsSearchTerms = false; }
					 */
